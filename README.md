...
* Enhanced scatter plotting with a fitted trend line option and updated the default scatter `fmt` to a dot marker.
...
* Completed plotting style API naming cleanup: `fmt`/`format` naming was iterated and finalized to `fmt`, QQ-plot now uses `marker`, and marker arguments are restricted to valid Matplotlib marker literals.
