...
## 🕒 History
### v0.1.12
* Added rank-normalization support via `PlottableData.ranknorm(...)`, using normal-
quantile mapping for percentile ranks.
* Optimized rank computation by vectorizing tie handling in `PlottableData.rank(...)`.
* Completed the naming migration from `PlotDataSet`/`PlottableDatas` to
`PlottableData`/`PlottableDataSet` for clearer API consistency.
* Refactored `PlottableData` by moving its data-processing logic into a new parent class
`Data`, keeping only plotting settings.

### v0.1.11
* Added arithmetic operator support to `MultiObject` (including reflected operators),
enabling element-wise math workflows for grouped `PlottableData` objects with length
checks.
* Improved numerical robustness in `utils.math` by consistently sanitizing `NaN`/`Inf`
inputs and validating finite sample counts in 1D linear regression.
* Refined `PlottableData.plot()` / `PlottableData.scatter()` x-tick handling to accept
broader array-like inputs and convert them consistently.
* Added and expanded unit tests for core/container behavior and utility modules.

### v0.1.10
* Renamed `dist_or_sample=` to `baseline=` in `PlottableData.qqplot()` for clearer
baseline specification.
* Removed `edge_precision=` from `PlottableData.ppplot()` and `PlottableData.ksplot()`,
and refined probability-range handling in the related diagnostic plotting flow.
* Improved QQ/PP plot readability by updating default axis labels and ensuring the
rightmost x-axis tick label is preserved.

### v0.1.9
* New method `PlottableData.rank(pct=True)` for rank transformation, supporting
`pct=False` to return raw ranks.
* Updated scatter behavior with improved defaults and removed implicit x-axis sorting.
* Refactored plotting APIs to remove the `ax` constructor argument for a cleaner artist
construction flow.

### v0.1.8
* Improved naming inference in `dp.data(...)` when decorators from `validating` are
involved.
* Refined module exports around artist helpers to make wildcard-style artist imports
behave consistently.
* Refactored plot-setting fallback behavior to use `dp.defaults` as the global source of
defaults.
* Enhanced `get_setting(...)` (including overload/type-hint coverage) so default values
align with the corresponding setting types, and dict defaults are handled more safely.
* Removed legacy `set_default()` usage and simplified setting application paths across
artists/containers.
...
### v0.1.6
* New method `PlottableData.scatter()` to draw true scatter charts while keeping
`PlottableData.plot()` as line chart behavior.
* Improved automatic label inference for `dp.data(...)`, plotting labels, and x-axis
labels in interactive contexts.
* Plot builders now use deferred drawing; removed `dp.show()` and improved axis/figure
rendering in object representations.
* Refined rendering stability with fixes for empty-axis cleanup and figure re-rendering
in `FigWrapper.__repr__`.
...
### v0.1.4
* Fixed issue: incorrectly displayed histogram statistics when the x-label had been
modified by the user.

### v0.1.3
* Allowed users to set the plot-settings by kwargs in artist methods like
`PlottableData.hist()`, `PlottableData.plot()`, etc.
* New operation methods `PlottableData.signedpow()` and `PlottableData.log10()`.
* Renamed `PlottableData.signlog()` to `.signedlog()`; renamed `PlottableData.opclear()`
to `.undo_all()`; removed `PlottableData.opclear_records_only()`.
* New optional parameter `format_label=` for `PlottableData.set_plot()` to decide
whether to format the label when painting on the axes.
* When defining the data classes, used *dataclasses* instead of *attrs* for a faster
import.

### v0.1.2
* New methods `PlottableData.corrmap()`, `PlottableData.ppplot()`, and
`PlottableData.resample()`.
* New optional parameter `fmt=` for `PlottableData.plot()`, `PlottableData.qqplot()`,
`PlottableData.ppplot()`, and `PlottableData.ksplot()`.
* Bugfix.

### v0.1.1
* New module-level function `dp.show()`.
* New methods `PlottableData.qqplot()`, `PlottableData.ksplot()` and
`PlottableData.abs()`.
* All the plotting method (e.g., `.hist()`) will now return an `Artist` object instead
of None.
* New plot settings: `grid` and `grid_alpha`.
* Parameters of `FigWrapper.set_figure()`, `AxesWrapper.set_axes()` and
`PlottableData.set_plot()` are keyword-only now.
* The returns of `.set_figure()` and `.set_axes()` will be None (instead of `self`) to
avoid misunderstandings.
* New optional parameter `inplace=` for `PlottableData.set_plot()` to decide whether the
changes will happen in-place (which is the only option before) or in a new copy.
* Parameter `ticks=` for `PlottableData.plot()` can be set to a `PlottableData` object
now.

### v0.1.0
* `PlottableData` now supports binary operations including +, -, *, /, and **.
* New methods `FigWrapper.set_figure()` and `AxesWrapper.set_axes()` - use them instead
of `*.set_plot()`.
* Simplified the usage of `AxesWrapper`.
* New plot settings: `subplots_adjust=`, `fontdict=` and `dpi=`.
* After this version, the required Python version is updated to >=3.11.9. Download and
install v0.0.2 if the user is under lower Python version (>=3.8.13).
